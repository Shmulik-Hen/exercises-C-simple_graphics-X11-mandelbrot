...
#define HEX(n, w)                                                              \
	"0x" << std::hex << std::setw((w)) << std::setfill('0') << std::right  \
	     << (n)
#define DEC(n, w)                                                              \
	std::dec << std::setw((w)) << std::setfill(' ') << std::right << (n)
#define DBL(n, p) std::setprecision(p) << std::fixed << (n)
#define STR(s, w) std::setw((w)) << std::setfill(' ') << std::left << (s)
#define ERR(s) std::cerr << STR(ERR_PFX, 1) << s << ENDL << std::flush
#define WARN(s) std::cerr << STR(WARN_PFX, 1) << s << ENDL << std::flush
