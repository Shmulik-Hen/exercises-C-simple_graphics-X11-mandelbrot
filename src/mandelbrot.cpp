...
mandelbrot::mand_data def_data = {DEF_ITERS, DEF_WIDTH, DEF_HEIGHT, DEF_LEFT,
				  DEF_RIGHT, DEF_TOP,	DEF_BOTTOM, DEF_LIMIT};

mandelbrot::mandelbrot() { _data = def_data; };
...
mandelbrot::mandelbrot(mand_data &d) {
	...
	DBG("mandelbrot:" << ENDL << STR("  iterations:", 14)
			  << DEC(_data.iterations, 4) << ENDL
			  << STR("  width:", 14) << DEC(_data.width, 4) << ENDL
			  << STR("  height:", 14) << DEC(_data.height, 4)
			  << ENDL << STR("  left:", 14) << DBL(_data.left, 6)
			  << ENDL << STR("  right:", 14) << DBL(_data.right, 6)
			  << ENDL << STR("  top:", 14) << DBL(_data.top, 6)
			  << ENDL << STR("  bottom:", 14)
			  << DBL(_data.bottom, 6) << ENDL << STR("  limit:", 14)
			  << DBL(_data.limit, 6) << ENDL);
	...
	DBG("mandelbrot:" << ENDL << STR("  x range:", 14) << DBL(_xrange, 15)
			  << ENDL << STR("  y range:", 14) << DBL(_yrange, 15)
			  << ENDL << STR("  x step:", 14) << DBL(_xstep, 18)
			  << ENDL << STR("  y step:", 14) << DBL(_ystep, 18)
			  << ENDL);
};
...
void mandelbrot::compute(plane_t &p) {
	...
	for (y = 0, y_pos = _data.top; y < p.size() && y_pos > _data.bottom;
	     y++, y_pos -= _ystep) {
		for (x = 0, x_pos = _data.left;
		     x < p[y].size() && x_pos < _data.right;
		     x++, x_pos += _xstep) {
			point pt = {x_pos, y_pos};
			p[y][x] = is_in_set(pt);
			DBG("iter: p[" << DEC(y, 1) << "][" << DEC(x, 1)
				       << "] = " << DEC(p[y][x], 4) << SEP
				       << DBL(y_pos, 6) << SEP
				       << DBL(x_pos, 6));
		}
